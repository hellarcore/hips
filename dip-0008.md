...
This HIP introduces ChainLocks, a technology for near-instant confirmation
...
only valid form of double spending possible in Hellar (InstantSend is not
double-spendable even for this case) and most other Bitcoin based
cryptocurrencies.

This HIP proposes a new method, called ChainLocks, for reducing uncertainty
when receiving funds and removing the possibility of 51% mining attacks.

## Prior work

* [HIP 006: Long Living Masternode Quorums](https://github.com/hellarcore/dips/blob/master/dip-0006.md)
* [HIP 007: LLMQ Signing Requests / Sessions](https://github.com/hellarcore/dips/blob/master/dip-0007.md)
...
(as found in Hellar Core) will be changed to honor this.
...
This assumes that [HIP-0010 LLMQ based InstantSend](https://github.com/hellarcore/dips/blob/master/dip-0010.md)
...
HIP-0010 also implements "retroactive signing of transactions", which guarantees that transactions
...
`SignIfMember` operation](https://github.com/hellarcore/dips/blob/master/dip-0007.md#void-signifmemberuint256-id-uint256-msghash-int-activellmqs).
...
`HasRecoveredSig` operation](https://github.com/hellarcore/dips/blob/master/dip-0007.md#bool-hasrecoveredsiguint256-id-uint256-msghash-int-activellmqs).
...
operation](https://github.com/hellarcore/dips/blob/master/dip-0007.md#bool-ismajoritypossibleuint256-id-uint256-msghash-int-activellmqs) must be used. An attempt
...
the [DIP007 `GetMostSignedSession` operation](https://github.com/hellarcore/dips/blob/master/dip-0007.md#uint256-getmostsignedsessionuint256-id-int-activellmqs), which is the hash of the block
...
will update this HIP and implementations accordingly.
...
use the [quorum_attack.py script](dip-0008/quorum_attack.py) to effectively
...
It is recommended that Hellar users require some transparency
...
Copyright (c) 2018 Hellar Core Inc. [Licensed under the MIT
