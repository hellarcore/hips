...
In Hellar use of the [Dark Gravity Wave (DGW)](https://docs.hellar.io/en/stable/introduction/features.html#dark-gravity-wave) difficulty adjustment algorithm results in nBits generally changing in every block. Consequently, this field will usually contain the full value.
...
A new service bit is required so that the nodes can advertise their ability to supply compact headers. Hellar would use bit 11 and designate it `NODE_HEADERS_COMPRESSED`.
...
Upon receipt of this message, the node is permitted, but not required, to preemptively announce new headers with the `headers2` message (instead of `inv`). Preemptive header announcement has been supported by the protocol version ≥ 70206 | Hellar Core version ≥ 0.12.1.
