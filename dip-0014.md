...
1. [The Dashpay Example](#the-hellarcore-example)
...
be achieved with 31-bit children. We need unique children in HellarPay derivation paths. Unique
...
The derivation path for Dashpay relationships is discussed more in depth in the HellarPay
...
(hellarcore mainnet secret) or `dpmp` (hellarcore mainnet public) on mainnet, `dpts` (hellarcore testnet
secret) or `dptp` (hellarcore testnet public) on testnet or devnets.
