...
* [HIP 002: Special Transactions](https://github.com/hellarcore/hips/blob/master/hip-0002.md)
* [HIP 003: Deterministic Masternode Lists](https://github.com/hellarcore/hips/blob/master/hip-0003.md)
...
LLMQs are highly dependent on the BLS Signature Scheme and more advanced BLS schemes. The following supporting documents describe these topics. It is advised to first read and understand these before continuing with this HIP.

* [BLS Signature Scheme](hip-0006/bls_signature_scheme.md)
* [BLS M-of-N Threshold Scheme and Distributed Key Generation](hip-0006/bls_m-of-n_threshold_scheme_and_dkg.md)
...
<p align="center"><img src="hip-0006/intra-quorum-comm.png" alt="Building intra-quorum connections" width="500"/></p>

Most messaging between members is performed using the inventory system of Hellar. This means that members will first send INV messages to other members and respond to GETDATA messages. Some communication messages, however, will be directly sent (see [DIP007 - LLMQ Signing Requests/Sessions](https://github.com/hellarcore/hips/blob/master/hip-0007.md)) to quorum members without using the inventory system.
...
Note: This process is modified for [Deterministic InstantSend](hip-0022.md) quorums which use the quorum rotation design specified in [DIP24](hip-0024.md).
...
The internal Hellar message name is `qcontrib` and the format of the message is:
...
The internal Hellar message name is `qcomplaint` and the format of the message is:
...
The internal Hellar message name is `qjustify` and the format of the message is:
...
The internal Hellar message name is `qpcommit` and the format of the message is:
...
The internal Hellar message name is `qfcommit` and the format of the message is:
...
`qfcommit` with version 2 or 4 include the `quorumIndex` field highlighted below as described in the [DIP24 Quorum Composition section](hip-0024.md#quorum-composition):
...
A merkle root of all active LLMQ sets is calculated and included into [DIP4 CbTx](https://github.com/hellarcore/hips/blob/master/hip-0004.md) with version >= 2.

## Signing sessions/requests

The details of the signing process are handled in [DIP007](https://github.com/hellarcore/hips/blob/master/hip-0007.md). The individual use cases for LLMQs are handled in specialized HIPs.

## Current LLMQ types

LLMQ types may be defined based on future HIPs. A list of registered LLMQ types can be found [here](hip-0006/llmq-types.md).
